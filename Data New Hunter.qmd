...
```{r}
# Convert the 'first_day_of_month' to a Date object if not already
analysis_data$first_day_of_month <- as.Date(analysis_data$first_day_of_month)

# Create a factor variable for plotting to distinguish the groups
analysis_data$group <- with(analysis_data, factor(paste(high_earning, post_scorecard),
